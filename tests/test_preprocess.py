...
from src.data.preprocess import build_report, preprocess_fred, preprocess_sec
...
def raw_sec_df() -> pd.DataFrame:
    """Return a minimal raw SEC DataFrame matching the raw layer schema."""
    return pd.DataFrame(
        {
...
def raw_fred_df() -> pd.DataFrame:
    """Return a minimal raw FRED DataFrame."""
    return pd.DataFrame(
        {
...
class TestPreprocessSec:
    """Tests for SEC preprocessing logic."""

    def test_cik_zero_padded(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify CIK is zero-padded to 10 digits."""
        result = preprocess_sec(raw_sec_df)
        assert all(len(c) == 10 for c in result["cik"]), "CIK should be zero-padded to 10 digits"

    def test_drops_null_fiscal_year(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify rows with null fiscal_year are dropped."""
        df = raw_sec_df.copy()
        df.loc[0, "fiscal_year"] = None
        result = preprocess_sec(df)
        assert result["fiscal_year"].isna().sum() == 0

    def test_filters_invalid_fiscal_period(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify invalid fiscal_period values like Q5 are removed."""
        df = raw_sec_df.copy()
        df.loc[0, "fiscal_period"] = "Q5"
        result = preprocess_sec(df)
        assert "Q5" not in result["fiscal_period"].values

    def test_valid_fiscal_periods_kept(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify only Q1-Q4 and FY fiscal periods remain."""
        result = preprocess_sec(raw_sec_df)
        valid = {"Q1", "Q2", "Q3", "Q4", "FY"}
        assert set(result["fiscal_period"].unique()).issubset(valid)

    def test_deduplication_keeps_latest(self, raw_sec_df: pd.DataFrame) -> None:
    ...
    def test_quarter_key_format(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify quarter_key matches <fiscal_year>_<fiscal_period>."""
        result = preprocess_sec(raw_sec_df)
        for _, row in result.iterrows():
            expected = f"{row['fiscal_year']}_{row['fiscal_period']}"
            assert row["quarter_key"] == expected

    def test_filters_fiscal_year_out_of_range(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify fiscal_year outside 1990-2030 is dropped."""
        df = raw_sec_df.copy()
    ...
    def test_drops_quarter_end_date_if_mostly_null(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify quarter_end_date column is dropped when mostly null."""
        df = raw_sec_df.copy()
        df["quarter_end_date"] = None  # 100% null
        result = preprocess_sec(df)
        assert "quarter_end_date" not in result.columns

    def test_output_has_no_nulls_in_key_columns(self, raw_sec_df: pd.DataFrame) -> None:
        """Verify key columns have no null values after preprocessing."""
        result = preprocess_sec(raw_sec_df)
...
class TestPreprocessFred:
    """Tests for FRED preprocessing logic."""

    def test_deduplication(self, raw_fred_df: pd.DataFrame) -> None:
    ...
    def test_date_snapped_to_quarter_end(self, raw_fred_df: pd.DataFrame) -> None:
        """Verify all dates are snapped to quarter-end."""
        result = preprocess_fred(raw_fred_df)
        for d in result["date"]:
            assert d == d + pd.offsets.QuarterEnd(0), f"{d} is not quarter-end"

    def test_sorted_ascending(self, raw_fred_df: pd.DataFrame) -> None:
        """Verify dates are sorted in ascending order."""
        result = preprocess_fred(raw_fred_df)
        dates = result["date"].tolist()
        assert dates == sorted(dates)

    def test_drops_null_dates(self, raw_fred_df: pd.DataFrame) -> None:
        """Verify rows with null dates are dropped."""
        df = raw_fred_df.copy()
        df.loc[0, "date"] = None
        result = preprocess_fred(df)
        assert result["date"].isna().sum() == 0

    def test_value_is_numeric(self, raw_fred_df: pd.DataFrame) -> None:
        """Verify value column is numeric type."""
        result = preprocess_fred(raw_fred_df)
        assert result["value"].dtype in ["float64", "float32", "int64"]

    def test_series_id_preserved(self, raw_fred_df: pd.DataFrame) -> None:
        """Verify series_id column is present and correct."""
        result = preprocess_fred(raw_fred_df)
...
class TestBuildReport:
    ...
    def test_report_structure(self, raw_sec_df: pd.DataFrame, raw_fred_df: pd.DataFrame) -> None:
        """Verify report contains expected keys and counts."""
    ...
    def test_report_is_json_serializable(
        self, raw_sec_df: pd.DataFrame, raw_fred_df: pd.DataFrame
    ) -> None:
        """Verify report can be serialized to JSON without errors."""
        sec = preprocess_sec(raw_sec_df)
        fred = preprocess_fred(raw_fred_df)
        report = build_report(sec, fred)
        # Should not raise
