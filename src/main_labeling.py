...
def main() -> None:
    ...
    df = read_parquet_from_gcs([f"gs://{settings.gcs_bucket}/{settings.panel_output_path}"])
    ...
    write_parquet_to_gcs(labeled_df, settings.gcs_bucket, settings.labeled_output_path)
